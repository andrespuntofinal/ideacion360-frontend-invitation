...
export default function Banner() {
    ...
    const { banner, paramsGeneral } = config;
    return (
        <section className="relative h-[80vh] w-full overflow-hidden">
            <motion.div initial={{ scale: 1.1 }} animate={{ scale: 1 }} transition={{ duration: 2 }} className="absolute inset-0">
                <video src={banner.videoDesktop} className="w-full h-full object-cover hidden md:block" autoPlay loop muted playsInline />
                <video src={banner.videoResponsive} className="w-full h-full object-cover block md:hidden" autoPlay loop muted playsInline />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                ...
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.8 }}
                    ...
                    style={{ color: banner.textColor, fontFamily: banner.subtitleFont }}>
                    {banner.subtextMsg}
                </motion.p>
            </div>
        </section>
