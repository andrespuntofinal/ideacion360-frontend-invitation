...
const EventDetail = () => {
  ...
  useEffect(() => { fetchEventById(id); }, [id]);

  const event = currentEvent;

  const formatDate = (d) => d ? new Date(d).toLocaleDateString('es-CO', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : '—';
    ...
    <AdminLayout>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem', color: 'var(--text-primary)' }}>
              {event.wedding?.coupleNames || 'Detalle del Evento'}
            </h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.25rem' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ padding: '1.5rem' }}>
          <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.9rem', color: 'var(--color-purple-light)', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <User size={16} /> Contacto
          </h3>
          {[
            { label: 'Nombre', value: event.contact?.name },
            { label: 'Email', value: event.contact?.email },
            { label: 'Celular', value: event.contact?.phone },
            { label: 'Identificación', value: event.contact?.identification },
          ].map(({ label, value }) => (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card" style={{ padding: '1.5rem' }}>
          <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.9rem', color: '#f472b6', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Heart size={16} /> Detalles de la Boda
          </h3>
          {[
            { label: 'Novios', value: event.wedding?.coupleNames },
            { label: 'Fecha', value: formatDate(event.wedding?.weddingDate) },
            { label: 'Hora', value: event.wedding?.weddingTime || '—' },
            { label: 'Tipo de Invitación', value: event.type === 'web' ? 'Boda Web' : event.type === 'video' ? 'Boda Video' : 'Boda Card' },
          ].map(({ label, value }) => (
            <div key={label} style={{ marginBottom: '0.75rem' }}>
              <div style={{ color: 'var(--text-muted)', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.2rem' }}>{label}</div>
              <div style={{ color: 'var(--text-primary)', fontSize: '0.9rem' }}>{value || '—'}</div>
        ...
        {/* Componentes */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card" style={{ padding: '1.5rem', gridColumn: 'span 1' }}>
          <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.9rem', color: 'var(--color-blue)', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Calendar size={16} /> Componentes ({activeComponents.length} activos)
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {Object.keys(COMPONENT_LABELS).map(key => {
              const isActive = event.activeComponents?.[key];
